...
export default async function TjpbProcessesPage({
  searchParams,
}: {
  ...
  try {
    response = await getTjpbProcesses(page, PAGE_SIZE);
  } catch (error) {
    return (
      <ErrorState
        message={getApiErrorMessage(error)}
        title="Erro ao carregar processos do TJPB"
      />
    );
  }
